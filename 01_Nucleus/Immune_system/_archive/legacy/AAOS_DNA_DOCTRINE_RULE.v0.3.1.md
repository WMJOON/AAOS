...
trigger: always_on
version: "0.3.1"
description: AAOS에서 모든 DNA 및 군체(Swarm) 구조가 반드시 준수해야 하는 교리 규칙(Doctrine). Natural Dissolution, Permission Principal, Auto-Enforcement, Audit Integrity, Multi-Agent Consensus를 포함한다.
---
# AAOS DNA Doctrine – RULE

본 문서는 AAOS의 모든 구조(DNA, 군체(Swarm), Skill, Workflow)가 위반할 수 없는 교리 규칙이다.
...
- `DNA.md`: 구조의 생성 조건, 성장 규칙, 종료 조건, 자원 상한, Natural Dissolution 절차를 포함해야 한다.
- 판정/감사 기록: Inquisitor의 판정 결과가 `AUDIT_LOG.md`에 남아야 한다.

### 1.2. DNA 파일 규칙

- 정식/변경 제안 모두 `DNA.md` 단일 파일에서 관리한다.
- 변경 제안은 별도 파일 분기 대신 동일 문서의 버전/합의 메타데이터로 관리한다.

### 1.3. 변경 제안 산출물 필수 스키마
...
- 정식 DNA 없이 새로운 구조를 생성/확장하는 행위
- 종료 조건(자연소멸)이 정의되지 않은 영구 구조
- 판정 불가능(검증 불가능)한 규칙/스킬/워크플로우
- **빈 값으로 스키마만 채운 형식적 제안 문서** (값이 실제로 존재해야 함)
...
실행 스크립트: `skills/core/dissolution_monitor.py`
...
Canon은 "정식 DNA 없이의 구조는 존재 자체가 허가되지 않는다"고 선언한다.
...
| 메커니즘 | 설명 | 구현 |
|----------|------|------|
| **Pre-Commit Hook** | Git commit 전 DNA/제안 산출물 검증 | `auto_inquisitor.py --gen-hook` |
...
실행 스크립트: `skills/core/auto_inquisitor.py`
...
- 대상(폴더/구조) 경로
- 판정 타입(judgment-dna(DNA 정통성) / permission / auto-enforcement / dissolution)
...
| 스크립트 | 경로 | 용도 |
|----------|------|------|
| `yaml_validator.py` | `core/` | YAML 파싱 및 값 검증 |
| `auto_inquisitor.py` | `core/` | 자동 검증, Hook 생성 |
| `dissolution_monitor.py` | `core/` | 자원 감시, 해체 실행 |
| `audit.py` | `core/` | 감사 로그, 무결성 검증 |
