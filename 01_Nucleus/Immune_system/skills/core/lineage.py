...
def _find_nearest_blueprint(start: Path) -> Optional[Path]:
    """
    Find nearest ancestor directory (including itself) containing DNA.md.
    """
    cur = start.resolve()
    if cur.is_file():
        cur = cur.parent
...
def _find_nearest_rule(start: Path, *, exclude: Optional[Path] = None) -> Optional[Path]:
    ...
    (Not AAOS root META Doctrine; this is node-local rule if present.)
    """
    cur = start.resolve()
    if cur.is_file():
        cur = cur.parent
...
def resolve_lineage(
    ...
    *,
    ...
    Severity controls how far upward the caller should consult:
      ...
      - medium: + immune doctrine + skills core
    ...
    meta_doctrine_path = (aaos_root / "00_METADoctrine" / "DNA.md").resolve()

    nearest_blueprint = _find_nearest_blueprint(target_path)
    # Exclude AAOS canonical META Doctrine from "node.rule" to avoid duplication.
    ...
    immune_doctrine_path = immune_root / "rules" / "README.md"
    skills_root = immune_root / "skills"
    audit_log_path = record_archive_root / "_archive" / "audit-log" / "AUDIT_LOG.md"
    meta_audit_log_path = record_archive_root / "_archive" / "meta-audit-log" / "META_AUDIT_LOG.md"
    ...
    if nearest_blueprint:
        ...
        nodes.append(
            LineageNode(
                ...
                False,
                note="Missing; structure is Non-Canonical until DNA.md exists.",
            )
        )
    ...
    if SEVERITY_ORDER.index(severity) >= SEVERITY_ORDER.index("medium"):
        nodes.extend(
            [
                ...
                LineageNode("immune.skills", skills_root, skills_root.exists()),
