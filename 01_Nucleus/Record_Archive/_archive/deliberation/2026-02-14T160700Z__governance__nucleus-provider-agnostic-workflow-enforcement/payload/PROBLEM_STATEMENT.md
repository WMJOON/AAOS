...
- KPI-2: 단계 2 기록(`Record_Archive/pending/*`)은 단계 1 `workflow_id`를 `record_path`/`issue_signature` 참조와 함께 남겨야 한다.
...
- KPI-5: Deliberation 진입 전 `01_Nucleus/AGENTS.md`의 1스텝 규칙 검증이 완료되어야 하며, 위반 항목이 있으면 `Request-Changes`로 즉시 중단되어야 한다.

검증 주기:
- 각 Nucleus 워크플로우 종료 시 `python3 01_Nucleus/scripts/nucleus_ops.py workflow-audit <manifest>` 실행으로 KPI-4~KPI-5를 기본 검증한다.
...
- 다음 액션: 
  1) 기존 준수 위반 후보를 `01_Nucleus/Record_Archive/`에서 검색한다.
     ...
     - 후보 산출물:
       ...
       - `01_Nucleus/Record_Archive/pending/`
  ...
  선행 증거(예시 1건):
  - `01_Nucleus/Deliberation_Chamber/plans/2026-01-24__record-archive-dna-v0.2.4-flagship-consensus/` 내 일부 계획이 plan/decomposition 분리 기록 없이 진행된 사례(문서 경로/시기 확인 필요)
