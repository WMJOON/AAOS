...
governance:
  ...
  sibling_ref: "04_Agentic_AI_OS/01_Nucleus/Immune_system/"
  precedence:
    - "AAOS Canon"
    - "Record Archive"
    - "META Doctrine"
...
meta_audit_log_reference: "04_Agentic_AI_OS/01_Nucleus/Immune_system/META_AUDIT_LOG.md"
...
7. **DNA 승격/변경의 플래그십 합의 (v0.2.4)**
   - 본 기관의 DNA(`DNA_BLUEPRINT.md` 또는 향후 `DNA.md`) 변경/승격은 **플래그십 Agent 2종 이상 합의**를 필수로 요구한다.
   - 합의는 Deliberation Chamber 산출물로 구조화되고, Record Archive에 패키지로 고정되어야 한다.
   - 근거: `04_Agentic_AI_OS/METADoctrine.md`의 Upper-Institution Change Gate 및 `01_Nucleus/Immune_system/AAOS_DNA_DOCTRINE_RULE.md`의 Multi-Agent Consensus Doctrine과 정렬.
...
- `indexes/HASH_LEDGER.md`
  ...
  - `hash`는 `SHA256(prev_hash + MANIFEST.sha256)` 값이다.
...
## DNA Promotion Gate: Flagship Consensus (v0.2.4)

Record Archive는 기관 순서상 상위(2순위)이며, 그 DNA 변경은 AAOS 전반의 “정통성 재현성”에 직접 영향을 준다.  
따라서 본 기관의 DNA 승격/변경은 플래그십 합의 규정을 강제한다.

### 적용 범위

- `04_Agentic_AI_OS/01_Nucleus/Record_Archive/DNA_BLUEPRINT.md`의 버전/정책/Hard Rules 변경
- `scripts/`(원장/검증 자동화)처럼 운영 무결성에 영향을 주는 변경
- (향후) `DNA.md`로 승격하거나, `DNA.md`를 갱신하는 변경

### 최소 요건 (MUST)

1. **플래그십 Agent 2종 이상 독립 검토**
   - 서로 다른 조직/모델 계열 2종 이상
   - 각 Agent는 `approve | reject | request-changes` verdict와 rationale를 남긴다
2. **Deliberation Packet**
   - `01_Nucleus/Record_Archive/templates/DELIBERATION_PACKET_TEMPLATE.md` 형식을 사용
3. **Record Archive 증빙 고정**
   - Deliberation Packet과 근거(스냅샷/리스크/결정 요약)를 `_archive/deliberation/` 또는 `_archive/snapshots/`에 패키지로 고정
4. **Immune System 판정/감사**
   - Inquisitor 심판 결과를 `01_Nucleus/Immune_system/AUDIT_LOG.md`에 남기고, 상위기관 변경이라면 `META_AUDIT_LOG.md`에도 참조를 남긴다

### 예외 (SHOULD, 사후 합의 필수)

- 무결성 파손/즉시 대응이 필요한 경우(예: 원장 체인 파손 복구):
  - 단일 Agent + 인간 서명으로 임시 조치 후, 7일 내 플래그십 2종 합의를 추가로 고정한다.
...
- v0.2.4 : Record Archive DNA 승격/변경에 플래그십 Agent 2종 합의 규정 추가(Deliberation→Archive→Immune 감사 정렬).
