...
- 다음 `context_for_next`: `01_Nucleus/deliberation_chamber/plans/_closed/2026-02-14__nucleus-provider-agnostic-workflow-enforcement/`에서 정비된 manifest를 기준으로 Stage-5 delibration revision 후보(체크리스트/작업 항목)를 생성하고, 실행 단계로 이동 전 `context_for_next`의 30건 점검 규칙을 시행한다.
