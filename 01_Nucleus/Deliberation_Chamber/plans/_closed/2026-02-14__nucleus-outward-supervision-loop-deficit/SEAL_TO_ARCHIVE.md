...
1. [x] `SEAL_TO_ARCHIVE.md` 산출물 상태를 `status: done`으로 확정한다.
2. [x] `plan_id` 관련 산출물을 모두 봉인 패키지 `payload/` 또는 `PAYLOAD/`에 복사한다.
3. [x] `SEAL_TO_ARCHIVE.md` 내부의 완료 타임스탬프와 대상 경로를 실 값으로 갱신한다.
4. [x] `01_Nucleus/record_archive/indexes/ARCHIVE_INDEX.md` 및 `01_Nucleus/record_archive/indexes/HASH_LEDGER.md` 갱신 유무를 기록한다.
5. [x] 해당 경로에 본 계획 패키지 산출물을 포함한다.
   (`WORKFLOW_MANIFEST`, `PROBLEM_STATEMENT`, `DELIBERATION_PACKET`, `CRITIQUE*`, `IMMUNE-CRITIQUE`, `SYNTHESIS`, `DECOMPOSITION_TODO`, `EVIDENCE`, `SEAL_TO_ARCHIVE`, `01_Nucleus/deliberation_chamber/tasks/...`).
6. [x] `PLANS_REGISTRY.md`와 plan 상태를 동기화한다.
