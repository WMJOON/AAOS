...
- **Why**: 인간은 암묵적 합의로 폴더를 쓰지만, 에이전트는 명시적 텍스트(`README.md`, `skills/00.pointerical-tooling/references/cof-environment-set.md`) 없이는 폴더의 의도를 추론해야 하므로 오류가 발생한다.
- **Action**: 핵심 노드는 반드시 스킬 기반 규약 문서(`skills/00.pointerical-tooling/references/cof-environment-set.md`)를 참조하여 "이 공간의 법칙"을 선언한다.
