...
def _detect_cof_root(start_path: str) -> Optional[str]:
    ...
    while True:
        ...
        if (current / "COF_DOCTRINE.md").is_file():
            return str(current)
        if (current / "skills" / "01.glob-indexing" / "scripts" / "cof_glob_indexing.py").is_file():
            return str(current)
...
def run_context_indexing(
    ...
    script_path = os.path.join(cof_root, "skills", "01.glob-indexing", "scripts", "cof_glob_indexing.py")
