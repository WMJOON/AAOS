...
본 문서는 COF(Context-Orchestrated Filesystem)에서 작업 맥락을 관리하기 위한 `NN.agents-task-context/` 노드(legacy: `task-manager/`)의 생성·검증·티켓 발행·아카이빙을 수행하는 스킬 `cof-task-manager-node`의 스펙을 정의한다.
