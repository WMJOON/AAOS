#!/usr/bin/env python3
"""
task-ticket/ 노드 생성 스크립트

Usage:
    python create_node.py <target_path>
    python create_node.py <target_path> --with-issue-notes
    python create_node.py <target_path> --with-release-notes
...
def create_task_ticket_node(
    ...
    node_path = target / "task-ticket"
    result["path"] = str(node_path)

    # 이미 존재하는지 확인
    if node_path.exists():
        result["errors"].append(f"task-ticket/ already exists at: {node_path}")
        return result

    try:
        # 기본 구조 생성
        node_path.mkdir()
        result["created"].append("task-ticket/")

        (node_path / "tickets").mkdir()
        result["created"].append("task-ticket/tickets/")

        create_rule_md(node_path)
        result["created"].append("task-ticket/RULE.md")

        create_troubleshooting_md(node_path)
        result["created"].append("task-ticket/troubleshooting.md")

        # 선택적 하위 노드
        if with_issue_notes:
            create_issue_notes(node_path)
            result["created"].append("task-ticket/issue_notes/")
            result["created"].append("task-ticket/issue_notes/RULE.md")

        if with_release_notes:
            create_release_notes(node_path)
            result["created"].append("task-ticket/release_notes/")
            result["created"].append("task-ticket/release_notes/RULE.md")
...
def main():
    parser = argparse.ArgumentParser(
        description="Create task-ticket/ node structure"
    )
    parser.add_argument(
        "target_path",
        help="Path where task-ticket/ will be created"
    )
    parser.add_argument(
        "--with-issue-notes",
        action="store_true",
    ...
    result = create_task_ticket_node(
        args.target_path,
    ...
    if result["success"]:
        print(f"OK: Created task-ticket/ at {result['path']}")
        print("\nCreated:")
