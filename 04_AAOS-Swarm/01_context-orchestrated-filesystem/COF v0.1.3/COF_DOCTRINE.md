...
> **Inheritance Note**: This doctrine inherits and evolves the principles defined in `COF v0.1.2/README.md`. It retains the core definition of "Context-Orchestrated Filesystem" while expanding the philosophical foundation for Agentic AI.
...
- **Why**: 인간은 암묵적 합의로 폴더를 쓰지만, 에이전트는 명시적 텍스트(`README.md`, `RULE.md`) 없이는 폴더의 의도를 추론해야 하므로 오류가 발생한다.
- **Action**: 핵심 노드에는 반드시 `RULE.md`를 배치하여 에이전트에게 "이 공간의 법칙"을 선언한다.
