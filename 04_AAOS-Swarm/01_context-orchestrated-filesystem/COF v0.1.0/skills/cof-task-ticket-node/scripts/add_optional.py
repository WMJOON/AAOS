#!/usr/bin/env python3
"""
기존 task-ticket/ 노드에 선택적 하위 노드 추가
...
def main():
    parser = argparse.ArgumentParser(
        description="Add optional child nodes to task-ticket/"
    )
    parser.add_argument(
        "node_path",
        help="Path to existing task-ticket/ directory"
    )
    parser.add_argument(
        "--issue-notes",
        action="store_true",
    ...
    if path.name != "task-ticket":
        print(f"WARNING: Expected 'task-ticket/', found '{path.name}/'")
